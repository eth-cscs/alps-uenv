...
class Ncl(Package):
    ...
    # ESMF is only needed at runtime for ESMF_regridding.ncl. It is a heavy
    # dependency (esmf + parallelio) that is not used at CSCS, so make it opt-in
    # and default it off. Upstream hard-codes it as a run dependency.
    variant("esmf", default=False, description="Enable ESMF regridding support (ESMF_regridding.ncl)")
    ...
    # CSCS: gated behind +esmf (default off) to drop the esmf/parallelio deps.
    depends_on("esmf+netcdf", type="run", when="+esmf")
    ...
    def patch(self):
        ...
        # NCL 6.6.2 ships stray debug calls to nc_set_log_level(3) that were
        # accidentally left uncommented in three nio backends (each sits right
        # below an identical commented-out copy). nc_set_log_level is only
        # exported by netcdf-c built with +logging, so the final ncl link fails
        # with "undefined reference to nc_set_log_level". These are pure debug
        # leftovers; drop them rather than force a netcdf-c+logging rebuild of
        # the whole (unified) stack.
        for nio_file in ("NclNetCdf.c", "NclNewHDF5.c", "NclNetCDF4.c"):
            filter_file(
                r"^(\s*)nc_set_log_level\(3\);",
                r"\1/* nc_set_log_level(3); removed: CSCS, see package.py */",
                join_path("ni", "src", "ncl", nio_file),
            )
        ...
        if self.spec.satisfies("+grib"):
            ...
            # CSCS/aarch64 fix: the shipped g2clib makefile hard-codes the
            # x86-only '-m64' flag and references the jasper include directory
            # without a '-I' prefix. On aarch64 gcc rejects '-m64' and treats
            # the bare include path as an input file, so libgrib2c.a is never
            # built and the final ncl link fails ("cannot find libgrib2c.a").
            # Add the '-I' prefix and drop '-m64' (the upstream for_aarch64.patch
            # only fixes the generated ymake config, not this static makefile).
            filter_file(
                "INC=.*",
    ...
    def setup_run_environment(self, env: EnvironmentModifications) -> None:
        ...
        if not self.spec.external and self.spec.satisfies("+esmf"):
