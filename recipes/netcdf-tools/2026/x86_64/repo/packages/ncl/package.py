...
class Ncl(Package):
    ...
    depends_on("jpeg")
    # NCL's NetCDF interface (NclNetCdf.c, NclNetCDF4.c, NclNewHDF5.c) calls
    # nc_set_log_level(), which netcdf-c only exports when built +logging.
    # Without it the ncl binary fails to link ("undefined reference to
    # nc_set_log_level"). Force the logging-enabled netcdf-c.
    depends_on("netcdf-c+logging")
    ...
    # ESMF is only required at runtime for the ESMF_regridding.ncl functions
    # (see https://www.ncl.ucar.edu/Applications/ESMF.shtml). CSCS does not use
    # NCL with ESMF models, and ESMF is expensive/fragile to build, so the
    # dependency is dropped here. Everything except the ESMF_regrid_* functions
    # works without it. NCL still runs fine; ESMFBINDIR is simply left unset
    # (see setup_run_environment below).
    ...
    def setup_run_environment(self, env: EnvironmentModifications) -> None:
        ...
        # Only set ESMFBINDIR when NCL was actually built against ESMF.
