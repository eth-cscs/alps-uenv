...
class Ferret(Package):
    ...
    def flag_handler(self, name, flags):
        if name == "fflags" and self.spec.satisfies("%gcc@10:"):
            flags.extend(["-fallow-argument-mismatch", "-fallow-invalid-boz", "-Wno-implicit-function-declaration"])
        if name == "cflags" and self.spec.satisfies("%gcc@10:"):
            flags.extend(["-Wno-implicit-function-declaration", "-Wno-implicit-int"])
    ...
    def patch(self):
        ...
        with working_dir(work_dir, create=False):
            ...
            # Ferret and its bundled xgks graphics library assume an x86_64
            # host: the x86_64-linux platform/CUSTOMIZE files hard-code the
            # "-m64" flag, which gcc rejects on aarch64 (neoverse). Strip it
            # from every build-config file so the ARM build succeeds. On x86_64
            # "-m64" is the default, so removing it there is a no-op.
            if spec.satisfies("target=aarch64:"):
                for root, _, files in os.walk("."):
                    for fn in files:
                        if (
                            fn.startswith("CUSTOMIZE")
                            or fn == "Makefile"
                            or fn == "config.status"
                            or fn.endswith(".mk")
                            or ".mk." in fn
                        ):
                            filter_file(r"-m64", "", os.path.join(root, fn), string=True)

    def install(self, spec, prefix):
        # libquadmath is an x86/ia64-only gcc runtime library; it does not
        # exist on aarch64, where linking "-lquadmath" fails. Only add it on
        # x86_64, where Ferret's gfortran objects reference its symbols.
