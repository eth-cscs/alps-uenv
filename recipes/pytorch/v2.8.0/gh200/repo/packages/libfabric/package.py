...
class Libfabric(AutotoolsPackage, CudaPackage, ROCmPackage):
    ...
    def patch(self):
        ...
        Make it match the libfabric we're building so that 2.3.x (and future)
        builds pick up the correct FI version.

        We only touch the file when building with the CXI provider enabled.
        """
        # Only adjust when cxi fabric is requested and the header exists
        if "fabrics=cxi" in self.spec:
            # Compute the libfabric MAJOR,MINOR from the spec (e.g., 2,3 for 2.3.1)
            try:
                major = int(self.spec.version[0])
                minor = int(self.spec.version[1])
            except Exception:
                # Fallback: do nothing if we can't parse version (e.g. @main)
                return

            cxi_header = join_path(self.stage.source_path, "prov", "cxi", "include", "cxip.h")

            # Replace any FI_VERSION(x, y) on the CXIP_FI_VERSION line with the right numbers
            # Example: "#define CXIP_FI_VERSION         FI_VERSION(2, 2)" -> "(2, 3)"
            filter_file(
                r'(#\s*define\s+CXIP_FI_VERSION\s+)FI_VERSION\(\s*\d+\s*,\s*\d+\s*\)',
                r'\1FI_VERSION(%d, %d)' % (major, minor),
                cxi_header,
                ignore_absent=True,
            )
