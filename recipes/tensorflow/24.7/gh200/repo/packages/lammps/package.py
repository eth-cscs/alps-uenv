...
class Lammps(CMakePackage, CudaPackage, ROCmPackage, PythonExtension):
    ...
    with when("+user-deepmd"):
        patch("deepmd.patch", when="@20230802.3:")
        depends_on("deepmd@2.2.11+tensorflow")
        depends_on("deepmd+horovod", when="+mpi")
        depends_on("deepmd+cuda", when="+cuda")
        depends_on("deepmd+rocm", when="+rocm")
