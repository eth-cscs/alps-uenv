...
class Gcc(AutotoolsPackage, GNUMirrorPackage, CompilerPackage):
    ...
    def patch(self):
        ...
        if spec.satisfies("+nvptx"):
            ...
            # gcc's nvptx target defaults to an sm_52 multilib (config.gcc:
            # nvptx_multilibs_default, and the with_arch fallback). CUDA 13.3's
            # ptxas dropped every arch below sm_75, so that sm_52 multilib fails
            # to assemble ("Value 'sm_XX' is not defined for option 'gpu-name'")
            # during configure-target-libgcc. Retarget both defaults to sm_80 so
            # the host offload build (which we can't pass configure flags to)
            # never emits an sm_52 multilib. sm_80 is valid for gcc 15's nvptx
            # (ceiling sm_89) and its PTX JIT-runs on the GH200 (Hopper) target.
            filter_file(
                "nvptx_multilibs_default=sm_52",
                "nvptx_multilibs_default=sm_80",
                "gcc/config.gcc",
                string=True,
            )
            filter_file(
    ...
    def nvptx_install(self):
        ...
        with working_dir("spack-build-nvptx", create=True):
            options = [
                ...
                "--target=nvptx-none",
                # nvptx offload default ISA. Two bounds constrain the value
                # (see gcc/config.gcc, nvptx-* case):
                #   floor:   CUDA 13.3's ptxas dropped everything below sm_75,
                #            so the default multilib must be >= sm_75 (without
                #            --with-arch it defaults to sm_52, which ptxas
                #            rejects -> configure-target-libgcc fails).
                #   ceiling: gcc 15's nvptx backend only accepts up to sm_89;
                #            sm_90 is *not* a valid --with-arch value here.
                # Target is GH200 (Hopper, cc 9.0), but gcc embeds PTX, which
                # the driver JIT-compiles forward-compatibly, so sm_80 PTX runs
                # natively-JIT'd on Hopper. sm_80 is valid, >= the CUDA floor,
                # and JIT-portable across Ampere/Ada/Hopper.
                # This is the CSCS-local patch to the upstream package.
                "--with-arch=sm_80",
                # Constrain the multilib set to sm_80 only. Without this, gcc
                # also builds an sm_52 multilib (TM_MULTILIB_CONFIG="sm_80 sm_52")
                # which CUDA 13.3's ptxas rejects. --with-multilib-list=sm_80
                # yields TM_MULTILIB_CONFIG=sm_80 (single base multilib). To
                # target several GPUs from one compiler instead, list them, e.g.
                # --with-multilib-list=sm_75,sm_80,sm_89 (all >= the CUDA 13.3
                # sm_75 floor and <= gcc 15's sm_89 ceiling).
                "--with-multilib-list=sm_80",
    ...
    def write_specs_file(self):
        ...
        if self.spec.satisfies("+nvptx"):
            ...
            if libgomp_dir:
                ...
                orig = libgomp_spec_file + ".orig"
                # Preserve the pristine file on the first run only, then always
                # regenerate from it. This method can run more than once on the
                # same prefix (e.g. @run_after("install") on a source build, then
                # again from _post_buildcache_install_hook when the package is
                # reinstalled from a buildcache). The old in-place edit was not
                # idempotent: it re-processed already-modified content, appending
                # a second "# Generated by Spack" block and a duplicate -rpath.
                # That produced a run of 3+ consecutive newlines, which gcc's own
                # spec parser rejects ("specs file malformed"), breaking *all*
                # -fopenmp linking. Regenerating from .orig with a truncating
                # write keeps the result identical no matter how often we run.
                if not os.path.exists(orig):
                    copy(libgomp_spec_file, orig)
                with open(orig) as f:
                    lines = f.readlines()
                with open(libgomp_spec_file, "w") as f:
                    for line in lines:
