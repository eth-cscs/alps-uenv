...
class QuantumEspresso(CMakePackage, CudaPackage, Package):
    ...
    variant("oscdft", default=False, description="Enable OSCDFT")
    ...
    # blas libraries have been removed from qe_lapack target in 7.4.1
            ...
            if spec.satisfies("+cuda"):
                ...
                cmake_args.append(
                    self.define(
                        "QE_GPU_ARCHS",
                        ";".join(
                            "sm_" + arch for arch in spec.variants["cuda_arch"].value
                        ),
                    )
                )
