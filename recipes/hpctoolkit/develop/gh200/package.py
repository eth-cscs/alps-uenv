...
class Hpctoolkit(AutotoolsPackage, MesonPackage):
    ...
    #     print(f"adding CUPTI cupti lib dir to LD_LIBRARY_PATH {cupti_dir}/lib64")
    #     env.append_path("PATH", os.path.join(cupti_dir, "lib64"))
    #     env.append_path("PATH", os.path.join(cupti_dir, "lib64"))
    #     env.append_path("LD_LIBRARY_PATH", os.path.join(cupti_dir, "lib64"))
    #     env.append_path("LD_LIBRARY_PATH", os.path.join(cupti_dir, "lib"))
...
class AutotoolsBuilder(autotools.AutotoolsBuilder):
    def configure_args(self):
        ...
        if spec.satisfies("+cuda"):
            cuda_prefix = os.path.realpath(spec["cuda"].prefix)
            args.append("--with-cuda=%s" % cuda_prefix)
...
class BuildEnvironment:
    def setup_build_environment(self, env: EnvironmentModifications) -> None:
        cuda_prefix = os.path.realpath(self.spec["cuda"].prefix)
        cupti_h = find_headers("cupti", cuda_prefix, recursive=True)
        cupti_dir = os.path.dirname(os.path.dirname(cupti_h[0]))
        p1 = os.path.join(cupti_dir, "lib")
        p2 = os.path.join(cupti_dir, "lib64")
        env.append_path("PATH", p1)
        env.append_path("PATH", p2)
        env.append_path("LD_LIBRARY_PATH", p1)
        env.append_path("LD_LIBRARY_PATH", p2)
        tty.msg(f"Adding CUPTI to PATH {p1} and {p2}")
...
class MesonBuilder(BuildEnvironment, meson.MesonBuilder):
    def meson_args(self):
        ...
        # Add CUDA include path to Meson include flags if CUDA is enabled
        if spec.satisfies("+cuda"):
            cuda_prefix = os.path.realpath(self.spec["cuda"].prefix)
            cupti_h = find_headers("cupti", cuda_prefix, recursive=True)
            cupti_dir = os.path.dirname(os.path.dirname(cupti_h[0]))
            args.append(f"-Dc_args=-I{cupti_dir}/include")
            args.append(f"-Dcpp_args=-I{cupti_dir}/include")
